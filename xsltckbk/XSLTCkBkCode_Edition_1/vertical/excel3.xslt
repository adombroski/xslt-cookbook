...
<xsl:stylesheet version="1.0" 
  ...
  <!-- If 0 then all worksheets will be processed. Otherwise the worksheet at this position will be processed. -->
    ...
    <xsl:element name="{concat($nsp,translate($topLevelName,' &#x9;&#xA;',$wsSub))}" namespace="{$namespace}">
      <xsl:choose>
        <xsl:when test="number($WSNum) > 0">
          <xsl:apply-templates select="ss:Worksheet[position() = number($WSNum)]/ss:Table">
